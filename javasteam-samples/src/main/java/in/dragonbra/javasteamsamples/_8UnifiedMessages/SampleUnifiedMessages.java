...
public class SampleUnifiedMessages implements Runnable {
    ...
    public void run() {
        ...
        steamClient.addHandler(new SteamUnifiedMessages());
    ...
    private void onLoggedOn(LoggedOnCallback callback) {
        ...
        // now let's send the request, this is done by building a classed based off the IPlayer interface.
        Player playerService = new Player(steamUnifiedMessages);
        favoriteBadge = playerService.GetFavoriteBadge(favoriteBadgeRequest.build());
        ...
        badgeRequest = steamUnifiedMessages.sendMessage(CPlayer_GetGameBadgeLevels_Request.class, "Player.GetGameBadgeLevels#1", badgeLevelsRequest);
    }
    ...
    private void onMethodResponse(ServiceMethodResponse callback) {
        ...
        if (callback.getJobID().equals(badgeRequest)) {
            CPlayer_GetGameBadgeLevels_Response.Builder response;
            response = callback.getDeserializedResponse(CPlayer_GetGameBadgeLevels_Response.class);
            response.build();

            System.out.println("Our player level is " + response.getPlayerLevel());

            List<CPlayer_GetGameBadgeLevels_Response.Badge> badgeList = response.getBadgesList();
            badgeList.forEach(badge -> System.out.println("Badge series " + badge.getSeries() + " is level " + badge.getLevel()));

            if (badgeList.isEmpty())
                System.out.println("No badges found for game. " + response.getBadgesCount() + ".");

            badgeRequest = JobID.INVALID;
        }

        if (callback.getJobID().equals(favoriteBadge)) {
            CPlayer_GetFavoriteBadge_Response.Builder response;
            response = callback.getDeserializedResponse(CPlayer_GetFavoriteBadge_Response.class);
            response.build();

            System.out.println("Has favorite badge: " + response.hasHasFavoriteBadge());
            System.out.println("Badge ID: " + response.getBadgeid());
            System.out.println("Community item ID: " + response.getCommunityitemid());
            System.out.println("Item Type: " + response.getItemType());
            System.out.println("Border Color: " + response.getBorderColor());
            System.out.println("App ID: " + response.getAppid());
