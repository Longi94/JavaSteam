...
open class AuthSession(
    ...
    suspend fun pollingWaitForResult(): AuthPollResult {
        ...
        when (preferredConfirmation.confirmationType) {
            ...
            // EAuthSessionGuardType.k_EAuthSessionGuardType_EmailConfirmation -> Unit // Unknown
            // EAuthSessionGuardType.k_EAuthSessionGuardType_MachineToken -> Unit // Unknown
    ...
    fun pollAuthSessionStatus(): AuthPollResult? {
        val request = CAuthentication_PollAuthSessionStatus_Request.newBuilder().apply {
            ...
            this.requestId = ByteString.copyFrom(requestID)
        }
        ...
        val response = message.getDeserializedResponse<CAuthentication_PollAuthSessionStatus_Response.Builder>(
            CAuthentication_PollAuthSessionStatus_Response::class.java
        ).build()

        handlePollAuthSessionStatusResponse(response)

        return if (response.refreshToken.isNotEmpty()) AuthPollResult(response) else null
    }
     ...
     */
    internal open fun handlePollAuthSessionStatusResponse(response: CAuthentication_PollAuthSessionStatus_Response) {
        if (response.hasNewClientId()) {
     ...
     */
    private fun sortConfirmations(
        confirmations: List<CAuthentication_AllowedConfirmation>,
    ): List<CAuthentication_AllowedConfirmation> {
        ...
        return confirmations.sortedBy { preferredConfirmationTypes[it.confirmationType] ?: Int.MAX_VALUE }
    }
}
